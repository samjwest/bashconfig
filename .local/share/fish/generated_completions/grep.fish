...
complete -c grep -l help --description 'Print a usage message briefly summarizing these command-line options and the …'
complete -c grep -s V -l version --description 'Print the version number of  grep to the standard output stream.'
complete -c grep -s E -l extended-regexp --description 'Interpret  PATTERN as an extended regular expression (ERE, see below).'
complete -c grep -s F -l fixed-strings --description 'Interpret  PATTERN as a list of fixed strings, separated by newlines, any of …'
complete -c grep -s G -l basic-regexp --description 'Interpret  PATTERN as a basic regular expression (BRE, see below).'
complete -c grep -s P -l perl-regexp --description 'Interpret  PATTERN as a Perl regular expression (PCRE, see below).'
...
complete -c grep -l mmap --description 'If possible, use the  mmap (2) system call to read input, instead of the defa…'
