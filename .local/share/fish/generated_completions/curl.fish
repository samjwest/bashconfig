...
complete -c curl -l 'http2.0' --description '(HTTP) Tells curl to issue its requests using HTTP 2. 0.'
complete -c curl -s 1 -l tlsv1 --description '(SSL) Forces curl to use TLS version 1 when negotiating with a remote TLS ser…'
...
complete -c curl -s 4 -l ipv4 --description 'If curl is capable of resolving an address to multiple IP versions (which it …'
complete -c curl -s 6 -l ipv6 --description 'If curl is capable of resolving an address to multiple IP versions (which it …'
complete -c curl -s a -l append --description '(FTP/SFTP) When used in an upload, this will tell curl to append to the targe…'
...
complete -c curl -l connect-timeout --description 'Maximum time in seconds that you allow the connection to the server to take.'
...
complete -c curl -l proxy-negotiate --description 'Tells curl to use HTTP Negotiate authentication when communicating with the g…'
...
complete -c curl -l socks5-gssapi-nec --description 'As part of the gssapi negotiation a protection mode is negotiated.'
...
complete -c curl -s u -l user --description 'Specify the user name, password and optional login options to use for server …'
...
complete -c curl -s v -l verbose --description 'Makes the fetching more verbose/talkative.  Mostly useful for debugging.'
complete -c curl -s w -l write-out --description 'Defines what to display on stdout after a completed and successful operation.'
