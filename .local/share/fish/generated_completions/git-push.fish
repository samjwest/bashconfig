...
complete -c git-push -l all --description 'Instead of naming each ref to push, specifies that all refs under refs/heads/…'
...
complete -c git-push -l repo --description 'This option is only relevant if no <repository> argument is passed in the inv…'
...
complete -c git-push -l recurse-submodules --description 'Make sure all submodule commits used by the revisions to be pushed are availa…'
