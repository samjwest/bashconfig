...
complete -c pager -s A -l SEARCH-SKIP-SCREEN --description 'Causes all forward searches (not just non-repeated searches)  to start just a…'
...
complete -c pager -s g -l hilite-search --description 'Normally,   less  will highlight ALL strings which match the last search comm…'
...
complete -c pager -s I -l IGNORE-CASE --description 'Like -i, but searches ignore case even if  the pattern contains uppercase let…'
...
complete -c pager -o ppattern -l pattern --description 'The -p option on the command line is equivalent to  specifying +/pattern; tha…'
complete -c pager -o Pprompt -l prompt --description 'Provides a way to tailor the three prompt styles to your own preference.'
complete -c pager -s q -l quiet -l silent --description 'Causes moderately "quiet" operation: the terminal bell is not rung  if an att…'
...
complete -c pager -s U -l UNDERLINE-SPECIAL --description 'Causes backspaces, tabs and carriage returns to be  treated as control charac…'
...
complete -c pager -s W -l HILITE-UNREAD --description 'Like -w, but temporarily highlights the first new line after any  forward mov…'
...
complete -c pager -l follow-name --description 'Normally, if the input file is renamed while an F command is executing,   les…'
...
complete -c pager -o Pw --description 'All prompt strings consist of a sequence of  letters and special escape seque…'
